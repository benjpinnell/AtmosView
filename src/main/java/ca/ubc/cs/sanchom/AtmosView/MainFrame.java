...
public class MainFrame extends JFrame {
	...
	private JTabbedPane getJTabbedPane() {
		if (jTabbedPane == null) {
			...
			jTabbedPane.addTab("Single View", null, getSingleViewPanel(), "Select and view individual soundings");
			jTabbedPane.addTab("Multi View", null, getMultiViewPanel(), "Select and view small multiples of soundings");
		}
		return jTabbedPane;
	}
	...
	private JPanel getMultiViewPanel()
	{
		if (MultiViewPanel == null)
		{
			...
			MultiViewPanel.addComponentListener(new java.awt.event.ComponentAdapter() {
	...
	private LegendPanel getLegendPanel() {
		if (legendPanel == null) {
			...
			legendPanel.addComponentListener(new java.awt.event.ComponentAdapter() {
				public void componentResized(java.awt.event.ComponentEvent e) {
	...
	private JScrollPane getStationScroller() {
		if (stationScroller == null) {
			...
			stationScroller.setBorder(BorderFactory.createCompoundBorder(
	...
	private JList getStationList() {
		if (stationJList == null) {
			...
			stationJList.addMouseListener(new java.awt.event.MouseAdapter() {
	...
	private JButton getActivateFileChooserButton() {
		if (ActivateFileChooserButton == null) {
			...
			ActivateFileChooserButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
	...
	private SoundingPanel getSoundingPanel()
	{
		if (SoundingDisplayPanel == null) {
			...
			SoundingDisplayPanel.addComponentListener(new java.awt.event.ComponentAdapter() {
				public void componentResized(java.awt.event.ComponentEvent e) {
	...
	private BarPanel getVisualizationPanel()
	{
		if (BarDisplayPanel == null) {
			...
			BarDisplayPanel.addComponentListener(new java.awt.event.ComponentAdapter() {
				public void componentResized(java.awt.event.ComponentEvent e) {
	...
	private void downloadSounding() {
		...
		if (selectedIndex >= 0 && selectedIndex < stationIDs.size())
		{
			...
			try {
				soundingData = SoundingFetcher.getInstance().getSounding(
						stationIDs.get(selectedIndex),
						chosen);
			} catch (IOException e) {
	...
	private void redrawMultiples()
	{
		...
		{
			BarPanel n = i.next();
			int dim = Math.min(MultiViewPanel.getHeight() / (multiples.size() <= 4 ? 1 : 2), MultiViewPanel.getWidth() / Math.min(multiples.size(), 4));
