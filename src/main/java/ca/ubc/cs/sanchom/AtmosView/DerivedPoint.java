...
/**
 * This class holds the derived/interpolated data points.
 * There is derived data not held as points in this class, however.
 * This only holds data that is meaningful for each sample point. Derived data that
 * is simply an altitude like convective condensation level is not held in this
 * structure.
...
public class DerivedPoint {
	...
	private final static double STRATUS_RH_THRESHOLD = 99; ///< Threshold for the indication of stratus cloud given humidity.
	...
	private double relativeHumidity; ///< The relative humidity calculated from the temperature and dewpoint
	private double vapourPressure; ///< The vapour pressure actually existant in the air at this point
	private double mixingRatio; ///< The mixing ratio at this point in g/kg
	private boolean stratusCloud; ///< Boolean flag indicating if the relative humidity is such to expect cloud.
	private double liftedParcelTemp; ///< The temperature of a parcel of air cooled adiabatically from the LCL upward
	private double liftedDiff; ///< The difference between the environmental temperature and the lifted parcel temperature.

	public DerivedPoint(double height, double pressure, double temperature, double dewpoint, double direction, double speed)
	{
		sampleHeight = height;
	...
	public int compareTo(Object obj)
	{
		if ( !(obj instanceof DerivedPoint) )
			throw new ClassCastException("Passed object cannot be compared with object of type DerivedPoint");
	...
	{
		return "pressure: " + pressure + "\theight: " + sampleHeight + "\tT: " + temperature + "\tD: " + dewpoint + "\tLT: " + liftedParcelTemp + "\tDiff: " + liftedDiff;
	}
}
